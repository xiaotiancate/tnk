				...
				<view class="item u-flex" v-for="(item, index) in couponList" :key="index">
					...
					<view class="content u-flex-1 u-flex">
						...
						<view class="right">
							...
							<view class="u-m-b-5 u-font-24 u-tips-color">每人限领{{ item.coupon && item.coupon.allow_num }}张</view>
							<view class="u-m-b-10 u-tips-color u-font-24">
								失效时间 : {{ item.expire_time|date('yyyy-mm-dd') }}
							</view>							
						</view>
					</view>
					<view class="right-btn u-p-l-30 u-p-r-30">
						...
						<block v-else-if="!item.begin"><u-button size="mini" :disabled="true" hover-class="none">未生效~</u-button></block>
						<block v-else>
							<u-button
								size="mini"
