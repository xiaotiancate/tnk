<template>
	<u-popup :show="show" @close="closeHandler">
		<view class="u-picker">
			<view style="width: 100%;position: relative;padding-top: 40rpx;">
				<text style="text-align: center;font-size: 30rpx;color: #555555;">{{title}}</text>
				<image src="@/static/icon/close.png" mode="aspectFill" @click="closeHandler"
					style="width: 35rpx;height: 35rpx;position: absolute;top: 37rpx;right: 40rpx;"></image>
			</view>
				...
				<picker-view-column v-for="(item, index) in innerColumns" :key="index" class="u-picker__view__column">
					<text v-if="$u.test.array(item)" class="u-picker__view__column__item u-line-1"
						v-for="(item1, index1) in item" :key="index1" :style="{
							...
							color: index1 === innerIndex[index] ? '#00B388' :'#333333'
