				...
				<view class="plr30">
					<swiper class="swiper" circular :autoplay="true" :interval="3000" :duration="1000">
						<swiper-item v-for="(item,index) in bannerList" :key="index">
							<image :src="item.thumb_image_text" mode="aspectFill" class="banner"></image>
						</swiper-item>
					</swiper>
				</view>
				<scroll-view scroll-x="true" class="scroll mt30">
					<view class="scroll_item flex-col flex-center" :class="[nindex==index?'active':'']"
						@click="onChangeService(index)" v-for="(item,index) in scrollList" :key="index">
						<image v-if="nindex==index" :src="item.image_text" mode="aspectFill"></image>
						<image v-else :src="item.image_text" mode="aspectFill"></image>
						<view class="mt5">{{item.name}}</view>
					</view>
				</scroll-view>
	...
	export default {
		...
		methods: {
			...
			fetchShop() {
				...
				let serviceId = this.scrollList[this.nindex].id;
