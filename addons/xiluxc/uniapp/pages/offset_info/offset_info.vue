	...
	<view>
		<view class="container bg-f5">
			<view class="p30">
				<view class="box1">
					...
					<view class="guide_item mt30 flex-box">
						<view class="flex-grow-1 pr30">
							<view class="fwb fs30 col1 m-ellipsis">{{order.shop.address}}</view>
				...
				<view class="box2 mt30" v-if="order.state==0">
					<view class="fwb fs30 col1">核销券码</view>
					<!-- 蒙层 -->
					<view class="code">
						<view :class="{moudle:order.qrcode.verifier_status==1}"></view>
						<image :src="order.qrcode.qrcode" mode="aspectFill" class="img"></image>
					</view>
					<view class="tc">
						<view class="number_box tl">
							券码<text class="pl30" :class="{tdl: order.qrcode.verifier_status==1}">{{order.qrcode.code}}</text>
						</view>
					</view>
				...
				<!-- <view class="box4 mt30 fs30 col1">
					...
					<view class="h_item" v-for="(item,index) in 3">
						...
						<view class="flex-box mt25">
							<image src="@/static/images/w_cover3.png" mode="aspectFill" class="user_img"></image>
							<view class="flex-grow-1 plr15 fs30 col5">幸启婷</view>
							<view class="fs24 col89 lh24">2025.01.10 12:20</view>
						</view>
					</view>
				</view> -->
	...
	export default {
		...
		methods: {
			fetchDetail(){
				...
				this.$core.get({url:'xiluxc.order/verify_detail',data:{order_id: this.orderId,lat: lat,lng:lng},success:(ret)=>{
					this.order = ret.data;
				}});
			},
			//导航
