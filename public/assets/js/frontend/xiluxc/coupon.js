define(['jquery', 'bootstrap', 'backend', 'table', 'form'], function ($, undefined, Backend, Table, Form) {
    ...
    var Controller = {
        index: function () {
            ...
            Table.api.init({
                extend: {
                    index_url: 'xiluxc/coupon/index' + location.search,
                    add_url: 'xiluxc/coupon/add',
                    edit_url: 'xiluxc/coupon/edit',
                    del_url: 'xiluxc/coupon/del',
                    multi_url: 'xiluxc/coupon/multi',
                    coupon_url: 'xiluxc/coupon/coupon_log',
                    table: 'xiluxc_coupon',
                }
            });
        ...
        coupon_log: function () {
            ...
            Table.api.init({
                extend: {
                    index_url: 'xiluxc/coupon/coupon_log'+location.search,
        ...
        api: {
            ...
            bindevent: function () {
                ...
                $(document).on('click','.btn-choose',function () {
                    ...
                    var url = value == 2 ? 'xiluxc/branch_package/selects' : 'xiluxc/branch_service/selects';
                    ...
                    Fast.api.open(url + "?shop_id="+shopId+"&ids="+selectids, __('Choose'), {
                        callback: function (data) {
                            ...
                            for(let i=0;i<data.data.length;i++){
                                html += '<tr >';
                                html += '<input type="hidden" name="row[items_ids][]" value="'+(value==1?data.data[i].shop_service_id:data.data[i].shop_package_id)+'">';
                                html += '<td><img width="40px" src="'+Fast.api.cdnurl(value==1?data.data[i].shop_service.image:data.data[i].shop_package.image)+'" alt=""></td>';
                                html += '<td>'+(value==1?data.data[i].service.name:data.data[i].shop_package.name)+'</td>';
