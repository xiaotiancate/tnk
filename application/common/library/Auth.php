...
class Auth
{
    ...
    public function register($username, $password, $email = '', $mobile = '', $extend = [])
    {
        ...
        try {
            ...
            $resuserinfo=Db::name('xiluxc_user_account')->where('user_id',$user->id)->find();
            
            if(!$resuserinfo){
                $loguserinfo['user_id']=$user->id;
                Db::name('xiluxc_user_account')->insert($loguserinfo);
            }
    ...
    public function login($account, $password)
    {
        ...
        if ($user->loginfailure >= 50 && time() - $user->loginfailuretime < 86400) {
            $this->setError('Please try again after 4 day');
            return false;
        }
