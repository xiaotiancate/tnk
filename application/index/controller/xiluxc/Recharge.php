 ...
 * @icon fa fa-circle-o
 */
class Recharge extends XiluxcFront
{

    /**
     * Recharge模型对象
     * @var \app\common\model\xiluxc\brand\Recharge
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\common\model\xiluxc\brand\Recharge;
    ...
    public function index()
    {
        ...
        $where2 = [];
        if($this->brand){
            $where2['recharge.brand_id'] = $this->brand->id;
        }else if($this->shop['type'] != 1){
            $brand = ShopBrand::get(['user_id'=>$this->shop->brand_id]);
            $where2['recharge.brand_id'] = $brand->id;
        }else{
