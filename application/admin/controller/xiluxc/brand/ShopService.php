...
class ShopService extends Backend
{
    ...
    public function selectpage()
    {
        ...
        if ($primaryvalue !== null) {
            $where = ['service.'.$primarykey => ['in', $primaryvalue]];
            $pagesize = 999999;
        } else {
        ...
        $list = [];
        $total = $this->model->with(['service'])->where($where)->count();
        if ($total > 0) {
            if (is_array($adminIds)) {
                $this->model->where($this->dataLimitField, 'in', $adminIds);
            }
            ...
            if ($primaryvalue !== null && preg_match("/^[a-z0-9_\-]+$/i", $primarykey)) {
                ...
                $this->model->orderRaw("FIELD(service.`{$primarykey}`, {$primaryvalue})");
            } else {
                $this->model->order($order);
            }

            $datalist = $this->model->with(['service'])->where($where)
                ->page($page, $pagesize)
                ->select();
            foreach ($datalist as $index => $item) {
                ...
                if ($this->selectpageFields == '*') {
                    $result = [
                        $primarykey => $item['service'][$primarykey] ?? '',
                        $field      => $item['service'][$field] ?? '',
                    ];
                } else {
    ...
    public function selects(){
        ...
        $list = $this->model
            ...
            ->whereExists(function ($query) use($currentName,$shopid){
                $shopBranchService = (new ShopBranchService())->getQuery()->getTable();
                $query->table($shopBranchService)->where($currentName.'.id=' . $shopBranchService . '.shop_service_id')->where("shop_id",$shopid);
            })
            ->order($sort, $order)
            ->paginate($limit);
        $result = ['total' => $list->total(), 'rows' => $list->items()];
